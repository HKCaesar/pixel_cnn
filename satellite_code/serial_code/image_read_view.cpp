...
double** init_taining_image()
{
    ...
    double**  from_image_to_memory(TIFF *image,\
                                  uint32 width,\
                                  uint32 height,\
                                  uint16 x_start,\
                                  uint16 y_start,\
                                  ...
                                  uint16 y_image_size)
    {
        ...
        pixel_image=new double*[x_image_size]();
        int r1,c1;
        for (r1 = x_start; r1 < height; r1++)
        {
            ...
            pixel_image[r1-x_start]=new double[y_image_size]();
    ...
    double*** from_image_to_memory(TIFF *image,uint32 width,uint32 height,uint16 SamplesPerPixel,uint16 x_start,uint16 y_start,uint16 x_image_size,uint16 y_image_size)
    {
        //cout<<width<<" "<<height<<" "<<SamplesPerPixel;
         double*** pixel_image=nullptr;
        uint16 line_size = SamplesPerPixel*width;
        uint16 *scanline = (uint16 *)_TIFFmalloc(line_size*sizeof(uint16 *));
        pixel_image=new double**[SamplesPerPixel]();
        ...
        for(s=0;s<SamplesPerPixel;s++)
        {
            ...
            {
                pixel_image[s][i]=new double[y_image_size]();
           ...
           // if(r1<x_start)
            ...
            for (c1 = y_start; c1 < (width); c1++)
            {
               ...
               // pixel_image[r1-x_start][c1-y_start]=new float[SamplesPerPixel]();
                ...
                {
                    //cout<<"Heloo";
                    pixel_image[s1][r1-x_start][c1-y_start]=scanline[c1*SamplesPerPixel+s1];
...
//          cout<<image_m_z;
          image_M=from_image_to_memory(M_image,image_m_x,image_m_y,image_m_z,x_start,y_start,image_size_x*M_x_fact,image_size_y*M_y_fact);
        ...
        image_3=from_image_to_memory(RGB_image,image_3_x,image_3_y,image_3_z,x_start,y_start,image_size_x*RGB_x_fact,image_size_y*RGB_y_fact);
        ...
        image_SWIR=from_image_to_memory(A_image,image_swir_x,image_swir_y,image_swir_z,x_start,y_start,image_size_x*A_x_fact,image_size_y*A_y_fact);
        //TIFFClose(image);
    }
    public:
